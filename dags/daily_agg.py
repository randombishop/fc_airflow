...
def assemble_results(**context):
    ...
    _, location1, job1 = xcom1.split(':', 2)
    _, location2, job2 = xcom2.split(':', 2)
    _, location3, job3 = xcom3.split(':', 2)
    bq_hook = BigQueryHook(gcp_conn_id='google_cloud_default')
    result_1 = bq_hook.get_query_results(job_id=job1, location=location1)
    result_2 = bq_hook.get_query_results(job_id=job2, location=location2)
    result_3 = bq_hook.get_query_results(job_id=job3, location=location3)
