...
CASE WHEN replied_to IS NULL THEN NULL ELSE 
  ARRAY(SELECT SUBSTRING(value, 3) FROM UNNEST(SPLIT(REGEXP_REPLACE(replied_to, r'\[|\]|\{|\}|\'', ''), ',')) AS value)
END AS replied_to,
CASE WHEN liked IS NULL THEN NULL ELSE 
  ARRAY(SELECT SUBSTRING(value, 3) FROM UNNEST(SPLIT(REGEXP_REPLACE(liked, r'\[|\]|\{|\}|\'', ''), ',')) AS value)
END AS liked,
CASE WHEN recasted IS NULL THEN NULL ELSE 
  ARRAY(SELECT SUBSTRING(value, 3) FROM UNNEST(SPLIT(REGEXP_REPLACE(recasted, r'\[|\]|\{|\}|\'', ''), ',')) AS value)
END AS recasted,
