...
COUNT(DISTINCT source_fid) FILTER (WHERE type=1) AS replies_ufids,
...
array_agg(DISTINCT source_fid) FILTER (WHERE type=1) AS replies_fids,
array_agg(DISTINCT source_fid) FILTER (WHERE type=3 AND reaction_type=1) AS likes_fids,
array_agg(DISTINCT source_fid) FILTER (WHERE type=3 AND reaction_type=2) AS recasts_fids
FROM msg
GROUP BY day, fid
